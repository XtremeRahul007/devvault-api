...
export function applyFilters(
  ...
  return files.filter((file) => {
    if (
      filters.name &&
      !file.originalName.toLowerCase().includes(filters.name.toLowerCase())
    ) {
      return false;
    }
    ...
    if (
      ...
      normalizeExtension(file.extension) !==
        normalizeExtension(filters.extension)
    ) {
      return false;
    }
...
export function applySorting(
  ...
  switch (sorting.sort) {
    case "name":
      return files.sort(
        (a, b) => a.originalName.localeCompare(b.originalName) * multiplier,
      );
