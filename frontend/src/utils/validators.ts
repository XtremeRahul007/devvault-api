...
export type ValidationResult =
  { ok: true; files: File[] } | { ok: false; reason: string };
...
const hasFiles = (files: File[]): ValidationResult => {
  return files.length > 0
    ? { ok: true, files }
    : { ok: false, reason: "No files selected." };
};
...
const withinTotalSize = (files: File[]): ValidationResult => {
  ...
  return totalSize <= MAX_FILE_SIZE
    ...
    : {
        ok: false,
        reason: `Total size exceeds the ${formatFileSize(MAX_FILE_SIZE)} limit.`,
      };
};

const rules: ValidationRule[] = [hasFiles, withinTotalSize];
...
export const validatedFiles = (fileList: FileList | null): File[] | null => {
  ...
  for (const rule of rules) {
    ...
    if (!result.ok) {
      ...
      toast.error(errorResult.reason);
